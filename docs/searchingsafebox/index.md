...
SearchingSafeBox 的 `plugin.yml` 依赖声明中要求 `Searching` 存在，因此需要先安装并启用 Searching。
...
8. 玩家破译成功，或自动破译完成后，调用 Searching API 打开原搜刮箱。
9. 会话被清理。
...
当前实现中，UI 调用 `context.success()` 后会直接打开原 Searching 搜刮箱，并清理会话。
